...
def map_companies_to_freelances(df_missions):
    ...
    for _, row in df_missions.iterrows():
        if row['company_id'] not in company_to_freelances.keys():
            company_to_freelances[row['company_id']] = {row['freelance_id']}
        elif row['company_id'] in company_to_freelances.keys():
...
def intersect_companies_sets(company_to_freelances, shared_freelances=10):
    ...
    # Dict company_pair_counts: assign to each pair of companies the count of freelances that worked for both of them
    company_pair_counts = {}
    
    # Set visited: used to avoid visiting twice each pair and to avoid comparing a company to itself.
    ...
    df_pairs = pd.DataFrame(company_pair_counts.items(), columns=['companies','pair_counts'])
    df_pairs[['company_a_id','company_b_id']] = pd.DataFrame(df_pairs['companies'].tolist())
...
def build_output(df_pairs, df_companies):
    ...
    df_temp = df_pairs.join(df_companies.set_index('company_id'), on='company_a_id', how='left')
    df_temp.rename(columns={'name':'company_a_name'}, inplace=True)
    df_temp = df_temp.join(df_companies.set_index('company_id'), on='company_b_id', how='left') 
    df_temp.rename(columns={'name':'company_b_name'}, inplace=True)

    # Build output dataframe
    df_final = df_temp.loc[:,['company_a_name','company_b_name','company_a_id','company_b_id','pair_counts']]
...
def task_solver(companies_path, missions_folder, shared_freelances):
    ...
    df_pairs = intersect_companies_sets(company_to_freelances, shared_freelances)
