...
def intersect_companies_sets(company_to_freelances, shared_freelances=10):
    ...
    df_pairs = pd.DataFrame(
        company_pair_counts.items(), columns=["companies", "pair_counts"]
    )
...
def build_output(df_pairs, df_companies):
    ...
    df_temp = df_pairs.join(
        df_companies.set_index("company_id"), on="company_a_id", how="left"
    )
    df_temp.rename(columns={"name": "company_a_name"}, inplace=True)
    df_temp = df_temp.join(
        df_companies.set_index("company_id"), on="company_b_id", how="left"
    )
    df_temp.rename(columns={"name": "company_b_name"}, inplace=True)

    # Build output dataframe
    df_final = df_temp.loc[
        :,
        [
            "company_a_name",
            "company_b_name",
